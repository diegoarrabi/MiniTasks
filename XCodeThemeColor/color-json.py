...
with open("json.txt", "r") as openfile:
    text_array = openfile.readlines()
    text_array = [line.strip().rstrip() for line in text_array if "#" in line]

for index, line in enumerate(text_array):
    line = line.replace('"foreground": ', "")
    line = line.replace("\"", "")
    line = line.replace("#", "")
    line = line.replace(",", "")
    line = line.upper()
    text_array[index] = f"{line}\n"

text_array = list(dict.fromkeys(text_array))
# for index, line in enumerate(text_array):
#     if index % 2 == 1:
#         continue
...
with open("json-format.txt", "w") as writefile:
    writefile.writelines(text_array)
